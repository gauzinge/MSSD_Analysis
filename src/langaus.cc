...
TF1 *langaufit(TH1F *his, double *fitrange, double *startvalues, double *parlimitslo, double *parlimitshi, double *fitparams, double *fiterrors, double* peak, double* peak_error, double gaus_sigma/*double *ChiSqr, Int_t *NDF*/)
{
    ...
    if (his->GetEntries() !=0) {
        ...
        //    ffit->GetParameters(par_var);
        //    for (int i = 0; i<4;i++) par_var[i]=fitparams[i];
...
//        double maxpeak = parfit->GetMaximumX(0, 100); // get distribution peak
...
//            double peakpos = parfit->GetMaximumX(0, 100);
            double peakpos = parfit->GetMaximumX(0, 48000);

            part_der[i] = (peakpos-maxpeak)/delta_par; // compute i-th partial derivative
            
            //        std::cout << "Peak " << maxpeak << " Parameter[" << i << "] = " << fitparams[i]  << " Param Error " << ffit->GetParError(i) << " delta Parameter[" << i << "] = " << delta_par << " new Peak Pos " << peakpos << " Difference = " << peakpos - maxpeak << " Partial derivative = " << part_der[i] << std::endl;
        }
        delete parfit;
...
TF1* langaus(TH1F* hSNR, double& peak, double& peak_error, double& mpv, double& mpv_error, double gaus_sigma) {
...
//    gStyle->SetOptFit(11111);
    ...
    // std::cout << "Landau MPV " << fitsnr->GetParameter(1) << " and Error " << fitsnr->GetParError(1) << " Peak " << peak << " and error " << peak_error << std::endl;
